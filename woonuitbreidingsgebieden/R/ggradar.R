...
ggradar2 <- function(plot.data,
    ...
    theme_clear <- theme_bw(base_size = base_size_in) +
    ...
    if (label.gridline.min==TRUE) {
        ...
        base <- base + geom_text(aes(x=x,y=y,label=values.radar[1]),data=gridline$min$label,size=grid.label.size*0.8, hjust=1) }
    base <- base + geom_text(aes(x=x,y=y,label=values.radar[2]),data=gridline$mid$label,size=grid.label.size*0.8, hjust=1)
    base <- base + geom_text(aes(x=x,y=y,label=values.radar[3]),data=gridline$max$label,size=grid.label.size*0.8, hjust=1)
    ...
    base <- base + theme(legend.key.width=unit(3,"line")) +
        theme(text = element_text(size = base_size_in)) +
        theme(legend.text = element_text(size = base_size_in),
